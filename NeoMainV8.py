...
def extract_rationale_procedure_facts(user_response):
    """
    Extract rationale, procedure, and facts from user response with scoring.
    Returns tuples of (description, score) for each dimension.
    """
    response_lower = user_response.lower()

    # Rationale assessment
    rationale_words = ["because", "reason", "therefore", "thus", "since", "due to", "consequently"]
    rationale_score = 0
    for word in rationale_words:
        if word in response_lower:
            rationale_score += 1
    rationale_score = min(5, rationale_score)  # Cap at 5

    if rationale_score > 3:
        rationale = ("Strong rationale with logical connections", rationale_score)
    elif rationale_score > 1:
        rationale = ("Basic rationale present", rationale_score)
    else:
        rationale = ("Limited rationale detected", rationale_score)

    # Procedure assessment
    procedure_words = ["steps", "procedure", "how to", "process", "method", "first", "second", "third", "finally"]
    procedure_score = 0
    for word in procedure_words:
        if word in response_lower:
            procedure_score += 1
    procedure_score = min(5, procedure_score)  # Cap at 5

    if procedure_score > 3:
        procedure = ("Well-defined procedures/methods", procedure_score)
    elif procedure_score > 1:
        procedure = ("Basic procedural knowledge", procedure_score)
    else:
        procedure = ("Limited procedural information", procedure_score)

    # Facts assessment
    facts_words = ["fact", "data", "statistic", "research", "study", "evidence", "according", "figure", "percentage"]
    facts_score = 0
    for word in facts_words:
        if word in response_lower:
            facts_score += 1
    facts_score = min(5, facts_score)  # Cap at 5

    if facts_score > 3:
        facts = ("Strong factual foundation", facts_score)
    elif facts_score > 1:
        facts = ("Basic factual information present", facts_score)
    else:
        facts = ("Limited factual support", facts_score)

    return rationale, procedure, facts


def generate_understanding_graph(rationale, procedure, facts, understanding_level):
    """
    Generate a radar chart showing understanding dimensions and improvement areas.
    Returns the figure and a list of improvement suggestions.
    """
    # Get scores from the tuples
    rationale_score = rationale[1]
    procedure_score = procedure[1]
    facts_score = facts[1]
    ...
    # Set the angles for each dimension (in radians)
    angles = np.linspace(0, 2 * np.pi, 3, endpoint=False).tolist()
    angles += angles[:1]  # Close the loop

    # Set the values to plot
    values = [rationale_score, procedure_score, facts_score]
    ...
    # Plot the values
    ax.plot(angles, values, 'o-', linewidth=2, label='Current Level')
    ax.fill(angles, values, alpha=0.25)

    # Plot the ideal values (all 5)
    ideal_values = [max_score] * 3
    ideal_values += ideal_values[:1]  # Close the loop
    ax.plot(angles, ideal_values, 'o-', linewidth=1, linestyle='--', label='Target Level')

    # Set the labels
    categories = ['Rationale', 'Procedure', 'Facts']
    ...
    ax.grid(True)
    plt.title(f'Understanding Profile (Level {understanding_level}/5)', size=15)
    plt.legend(loc='upper right')

    # Generate improvement suggestions based on the scores
    improvement_suggestions = []

    if rationale_score < 4:
        improvement_suggestions.append(
            "• Improve rationale by explaining 'why' and creating logical connections between concepts")

    if procedure_score < 4:
        improvement_suggestions.append("• Enhance procedural knowledge by outlining step-by-step processes and methods")

    if facts_score < 4:
        improvement_suggestions.append(
            "• Strengthen factual foundation by incorporating data, statistics, and evidence")

    return fig, improvement_suggestions
...
def evaluate_response(system_answer, user_response):
    ...
    if check_response_similarity(system_answer, user_response):
        return {
            ...
            "Rationale": ("No original rationale", 0),
            "Procedure": ("No original procedure", 0),
            "Facts": ("No original facts", 0),
        ...
        with st.form(key='evaluation_form'):
            ...
            if submit_evaluation and user_response:
                ...
                if isinstance(evaluation, dict):
                    ...
                    fig, improvement_suggestions = generate_understanding_graph(
                        rationale, procedure, facts, understanding_level
                    )

                    # Display the chart
                    st.markdown("### Understanding Profile")
                    st.pyplot(fig)

                    # Display knowledge dimension details
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.markdown(f"**Rationale: {rationale[1]}/5**")
                        st.write(rationale[0])
                    with col2:
                        st.markdown(f"**Procedure: {procedure[1]}/5**")
                        st.write(procedure[0])
                    with col3:
                        st.markdown(f"**Facts: {facts[1]}/5**")
                        st.write(facts[0])
                    ...
                    if len(st.session_state.evaluation_history) > 1:
                        ...
                        if len(history_df) > 1:
                            ...
                            attempts = range(1, len(history_df) + 1)
                            ax.plot(attempts, history_df['rationale_score'], 'o-', label='Rationale')
                            ax.plot(attempts, history_df['procedure_score'], 's-', label='Procedure')
                            ax.plot(attempts, history_df['facts_score'], '^-', label='Facts')
                            ax.plot(attempts, history_df['level'], 'D-', label='Overall Level')
                        ...
                        display_df = history_df[:-1][['timestamp', 'question', 'score', 'explanation', 'level']]

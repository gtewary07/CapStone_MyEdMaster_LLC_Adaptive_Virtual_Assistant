...
# Initialize Groq client (we'll set this up properly later)
client = None
try:
...
def init_session_state():
    ...
    if 'difficulty_level' not in st.session_state:
        st.session_state.difficulty_level = "Basic"
...
def get_llm_response(prompt, age, interest, experience, conversation_history=None, client=None, retries=3, delay=2):
    """
...
def adapt_content_based_on_assessment(topic, previous_results=None):
    ...
    if previous_results is None or not st.session_state.adaptive_mode:
        return st.session_state.difficulty_level, ""
    ...
    return adjusted_difficulty, focus_instruction
...
def add_adaptive_mode_toggle():
    ...
    adaptive_mode = st.sidebar.checkbox(
        ...
        help="When enabled, the system will adjust content difficulty based on your previous assessment results"
    )
    ...
    with tab2:
        ...
        with st.form(key='learning_form'):
            ...
            if submit_learning:
                ...
                if st.session_state.adaptive_mode and 'understanding_report' in st.session_state:
                    adapted_difficulty, focus_instruction = adapt_content_based_on_assessment(
                    ...
                    if adapted_difficulty != difficulty_level or focus_instruction:
                        adjustment_message = f"Based on your previous assessment results, the content has been adjusted:"
                        if adapted_difficulty != difficulty_level:
                            adjustment_message += f" Difficulty level changed to {adapted_difficulty}."
                        if focus_instruction:
                            dimensions = [("rationale", 0), ("factual", 0), ("procedural", 0)]
                            adjustment_message += f" Extra focus on {dimensions[0][0]} understanding."
